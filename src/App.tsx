      ...
      {view === "library" && <Panel title={t.library} eyebrow={EYEBROW.library[uiLang]}>
        ...
        {dicts.length > 0 && <>
          ...
          <div className="dict-grid">
            {dicts.map(d => <button key={d.id} className={source === d.id ? "dict-card active" : "dict-card"} onClick={() => selectDict(d)}>
              ...
              <div><b>{d.name}</b><p>{d.description}</p><small>{d.length} {uiLang === "id" ? "kata" : uiLang === "zh" ? "词" : "words"} · {d.lang === "zh" ? TX("英文 / 印尼语释义", "arti Inggris / Indonesia", "EN / ID glosses", uiLang) : TX("中文释义", "arti Mandarin", "Chinese glosses", uiLang)}</small></div>
              {source === d.id && <em>✓</em>}
            </button>)}
          </div>
        </>}
        ...
        {globalSearch
          ? (globalLoading
              ...
              : (!gq
                  ? <div className="empty"><b>🌐</b><h3>{uiLang === "zh" ? "输入关键词，在全部词库中搜索" : uiLang === "id" ? "Ketik untuk mencari di semua kamus" : "Type to search across all libraries"}</h3><p>{TX(`三语精选 + ${dicts.length} 个考试词库，共 ${words.length + dicts.reduce((a, d) => a + d.length, 0)} 词`, `Pilihan trilingual + ${dicts.length} kamus ujian · ${words.length + dicts.reduce((a, d) => a + d.length, 0)} kata`, `Trilingual collection + ${dicts.length} exam libraries · ${words.length + dicts.reduce((a, d) => a + d.length, 0)} words`, uiLang)}</p></div>
