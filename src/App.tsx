...
const MODAL_T: Record<Lang, { title: string; subtitle: string; ui: string; uiDesc: string; learn: string; learnDesc: string; def: string; defDesc: string; defRequired: string; defSame: string; none: string; noneDesc: string; selected: string; cancel: string; save: string }> = {
  zh: { title: "语言设置", subtitle: "配置界面语言、学习语言和释义语言", ui: "界面语言", uiDesc: "选择应用界面的显示语言", learn: "学习语言", learnDesc: "选择你要练习打字的语言", def: "释义用哪种语言", defDesc: "选你最熟悉的语言", defRequired: "请选一项再保存", defSame: "正在学的语言", none: "不显示释义", noneDesc: "只练打字", selected: "已选择", cancel: "取消", save: "保存设置" },
  id: { title: "Pengaturan Bahasa", subtitle: "Atur bahasa antarmuka, bahasa belajar, dan bahasa arti", ui: "Bahasa Antarmuka", uiDesc: "Pilih bahasa tampilan aplikasi", learn: "Bahasa Belajar", learnDesc: "Pilih bahasa yang ingin kamu latih mengetik", def: "Arti ditampilkan dalam", defDesc: "Pilih bahasa yang paling kamu pahami", defRequired: "Pilih salah satu sebelum menyimpan", defSame: "Bahasa yang sedang dipelajari", none: "Tanpa arti", noneDesc: "Hanya latihan mengetik", selected: "Dipilih", cancel: "Batal", save: "Simpan" },
  en: { title: "Language Settings", subtitle: "Choose interface, learning, and meaning language", ui: "Interface Language", uiDesc: "Language used for menus and labels", learn: "Learning Language", learnDesc: "The language you practice typing", def: "Show meanings in", defDesc: "Pick the language you read best", defRequired: "Pick one to save", defSame: "The language you are learning", none: "No meaning", noneDesc: "Typing only", selected: "Selected", cancel: "Cancel", save: "Save" },
};

const EYEBROW: Record<string, Record<Lang, string>> = {
  library: { zh: "词汇库", id: "Koleksi Kosakata", en: "WORD COLLECTIONS" },
  mistakes: { zh: "间隔复习", id: "Pengulangan Berjarak", en: "SPACED REPETITION" },
    ...
    <main className="main">
      ...
      {view === "mistakes" && <Panel title={t.mistakes} eyebrow={EYEBROW.mistakes[uiLang]}>
        ...
        <div className="mistake-list">{dueEntries.length ? dueEntries.map((x,i)=><button key={x.key} onClick={()=>jumpToKey(x.key)}><span>{i+1}</span><b>{x.info.text}</b><em className={x.info.note ? "meaning-missing" : undefined}>{defTag(x.info.label)}{x.info.meaning}</em><i>{TX("练习 →", "Latih →", "Practice →", uiLang)}</i></button>)
          : srs.due || mistakes.length ? <div className="empty"><b>◎</b><h3>{TX("错词本里没有可显示的词", "Belum ada yang bisa ditampilkan di sini", "Nothing to list here", uiLang)}</h3><p>{srs.due ? TX(`有 ${srs.due} 个词到期，点上方「开始复习」。`, `${srs.due} kata perlu diulang — tekan “Mulai mengulang” di atas.`, `${srs.due} ${srs.due === 1 ? "word is" : "words are"} due: press “Start review” above.`, uiLang) : TX("错过的词在别的词库里，打开那个词库后会显示。", "Kata yang salah ada di daftar lain; buka daftar itu untuk melihatnya.", "The missed words belong to another list; open it to see them here.", uiLang)}</p></div>
      ...
      {view === "stats" && <Panel title={t.stats} eyebrow={EYEBROW.stats[uiLang]}>
        ...
        <div className="backup-row"><div><b>{uiLang === "zh" ? "进度备份" : uiLang === "id" ? "Cadangan progres" : "Backup progress"}</b><small>{uiLang === "zh" ? "导出成文件，换设备可导入恢复（含错词、复习、收藏、设置）" : uiLang === "id" ? "Ekspor ke file, impor di perangkat lain (kata salah, jadwal pengulangan, favorit, pengaturan)" : "Export a file and import it on another device (mistakes, review schedule, favourites, settings)"}</small></div><div className="backup-btns"><button onClick={exportProgress}>{uiLang === "zh" ? "导出备份" : uiLang === "id" ? "Ekspor" : "Export"} ↓</button><label className="import-btn">{uiLang === "zh" ? "导入" : uiLang === "id" ? "Impor" : "Import"} ↑<input type="file" accept="application/json" onChange={e => { const f = e.target.files?.[0]; e.target.value = ""; if (f) importProgress(f); }} /></label></div></div>
      </Panel>}
      ...
      <div className="lang-modal-section">
        ...
        <div className="lang-cards">
          ...
          <button className={def === "none" ? "lang-card active" : "lang-card"} onClick={() => setPicked("none")}>
            <b>{mt.none}</b><small>{mt.noneDesc}</small>{def === "none" && <span>{mt.selected}</span>}
          </button>
        </div>
      </div>
